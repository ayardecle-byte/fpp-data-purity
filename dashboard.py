...
def crear_tabla_apuestas():
    # 👇 ASEGURAR QUE LA CARPETA EXISTA EN LA NUBE 👇
    os.makedirs("database", exist_ok=True)
    
    conn = sqlite3.connect("database/football_data.db")
    cursor = conn.cursor()
    # ... (el resto del código que ya tienes en esa función)
...
def cargar_tabla_json(nombre_liga):
    ...
    for row in data.get("posiciones", []):
        if isinstance(row, list) and len(row) >= 10:
            ...
            # 👇 CORRECCIÓN 2.0: Excepción VIP para All Boys 👇
    ...
    if not df.empty:
        ...
        # 👇 Agregamos "Estados Unidos - MLS" a la lista 👇
...
if st.session_state.pagina == 'Cartelera':
    ...
    if not tabla.empty:
        ...
        if liga_sel in ["Argentina", "Argentina - Primera Nacional", "Estados Unidos - MLS"]:
            ...
            "Estados Unidos - MLS", "México - Liga MX", 
        ...
        if st.session_state.get('analizar'):
            ...
            ligas_json_avanzadas = ["Brasil", "Brasil - Serie B", "Noruega - Eliteserien", "Argentina", "Argentina - Primera Nacional", "Estados Unidos - MLS", "México - Liga MX", "Estonia - Meistriliiga", "Islandia - 2da División", "Dinamarca - Superliga", "China - Super League", "Suecia - Allsvenskan", "Islandia - 1ra División"]
        ...
        # 👇 NUEVO: TERMÓMETRO DE TENDENCIAS EMPÍRICAS (Solo Localía y Visita) 👇
           ...
           # --- ANÁLISIS CONTEXTUAL ---
            ...
            # Recopilamos las cuotas que ingresaste para mandárselas a la IA
            cuotas_str = "\n".join([f"- {op['Mercado']}: Cuota {op['Cuota']:.2f}" for op in todas_ops if op['Cuota'] > 1.01])
            if not cuotas_str: 
                cuotas_str = "No se ingresaron cuotas de casas de apuestas para evaluar."

            prompt = f"""
¡ATENCIÓN! Hoy es {fecha_hoy}. Asume el rol de un Analista de Datos de Élite y Experto Profesional en Apuestas Deportivas. Tu reputación se basa en la precisión, la objetividad implacable y en saber identificar datos de primera calidad para encontrar "valor seguro".
...
2. Utiliza exclusivamente fuentes confiables en tu búsqueda interna (Flashscore, OneFootball, Sofascore, FootyStats, SoccerStats).
3. Eres un auditor crítico: NO estés de acuerdo con mi modelo estadístico solo por ser complaciente. Defiende tus propios datos con argumentos sólidos.

Escribe un informe estructurado, directo y analítico utilizando estas viñetas exactas:

🔍 PASO 1: INVESTIGACIÓN DE CAMPO (Contexto Actualizado)
Analiza y describe la situación actual de ambos equipos:
- Motivación y Contexto: (Peleas por título, descenso, rivalidades, cansancio por copas).
- Dinámica Reciente: (OBLIGATORIO: Menciona cómo les fue en sus ÚLTIMOS 5 PARTIDOS exactos).
- Rendimiento de Condición: (Cómo juega {l} exclusivamente en casa y {v} de visita).
- Bajas y Rotaciones: (Lesiones, suspensiones críticas actualizadas a la fecha).
- Estilo de Juego: (¿Quién dominará la posesión? ¿Defensas adelantadas? ¿Contragolpes?).

🧮 PASO 2: TUS PROPIAS PROBABILIDADES INDEPENDIENTES
Olvida mi modelo por un momento. Basado en tu investigación de campo, calcula TUS propias probabilidades porcentuales para:
- Ganador (1X2) y Doble Oportunidad.
- Goles Totales (+1.5, +2.5) y Goles por Equipo.
- Ambos Anotan (Sí / No).

⚔️ PASO 3: CONTRASTE CRÍTICO Y DEBATE (Tú vs Data Purity)
Mi modelo matemático crudo arrojó estos números:
- 1X2: Local {stats.get('1', 0):.1f}% | Empate {stats.get('X', 0):.1f}% | Visita {stats.get('2', 0):.1f}%
- Goles: +1.5 al {stats.get('Over15', 0):.1f}% | +2.5 al {stats.get('Over25', 0):.1f}%
- Ambos Anotan (Sí): {stats.get('BTTS_Y', 0):.1f}%
>> DEBATE: Compara tus resultados con los míos. ¿Mi modelo está sobrevalorando a un equipo por la historia, ignorando una racha reciente o una lesión clave? Destruye o valida mis números con argumentos reales.

⚖️ PASO 4: EVALUACIÓN DE CUOTAS Y VEREDICTO FINAL
He introducido las siguientes cuotas de mi casa de apuestas:
{cuotas_str}
- Escenarios: ¿Qué desarrollo de partido esperas ver?
- Riesgos: ¿Cuál es el mayor peligro de apostar aquí?
- VEREDICTO FINAL: Recomiéndame qué mercados tienen VALOR SEGURO basándote en las cuotas. Si las cuotas son basura, el partido es impredecible o hay demasiado riesgo, tu obligación profesional es ordenarme "NO APOSTAR EN ESTE PARTIDO (NO BET)".
"""
                    ...
                    with st.spinner("⏳ Analizando datos actuales en internet... Por favor espera un momento..."):
                        ...
                        try:
                            ...
                            if respuesta.status_code == 200:
                                ...
                                # 👇 LIMPIEZA DEL FORMATO (Evita el bloque oscuro horizontal) 👇
                                texto = texto.replace("```markdown", "").replace("```", "")
                                
                                st.success("✅ Análisis completado:")
                                st.markdown(texto) # Cambiamos write por markdown para un mejor diseño
                            else:
...
# ==========================================
    ...
    with col1:
        ...
        with st.expander("🇩🇰 Dinamarca (Superliga)", expanded=True):
            try:
                with open('data_json/denmark.json', 'r', encoding='utf-8') as f:
                    fix_den = json.load(f).get("fixture", [])
                if fix_den: st.dataframe(pd.DataFrame(fix_den), use_container_width=True, hide_index=True)
                else: st.info("No hay partidos programados.")
            except: st.warning("⚠️ Ejecuta el actualizador para cargar esta liga.")

        with st.expander("🇨🇳 China (Super League)", expanded=True):
            try:
                with open('data_json/china.json', 'r', encoding='utf-8') as f:
                    fix_chi = json.load(f).get("fixture", [])
                if fix_chi: st.dataframe(pd.DataFrame(fix_chi), use_container_width=True, hide_index=True)
                else: st.info("No hay partidos programados.")
            except: st.warning("⚠️ Ejecuta el actualizador para cargar esta liga.")
    ...
    with col2:
        ...
        with st.expander("🇸🇪 Suecia (Allsvenskan)", expanded=True):
            try:
                with open('data_json/sweden.json', 'r', encoding='utf-8') as f:
                    fix_swe = json.load(f).get("fixture", [])
                if fix_swe: st.dataframe(pd.DataFrame(fix_swe), use_container_width=True, hide_index=True)
                else: st.info("No hay partidos programados.")
            except: st.warning("⚠️ Ejecuta el actualizador para cargar esta liga.")

        with st.expander("🇮🇸 Islandia (1ra División)", expanded=True):
            try:
                with open('data_json/iceland.json', 'r', encoding='utf-8') as f:
                    fix_ice1 = json.load(f).get("fixture", [])
                if fix_ice1: st.dataframe(pd.DataFrame(fix_ice1), use_container_width=True, hide_index=True)
                else: st.info("No hay partidos programados.")
            except: st.warning("⚠️ Ejecuta el actualizador para cargar esta liga.")
# ==========================================
...
# ==========================================
    ...
    for _, row in df_apuestas.iterrows():
        ...
        # Fórmula de tu Excel: Stake = Cuota redondeada
    ...
    with colB:
        st.markdown("### 🧮 Calculadora de Inversión (Excel)")
        st.caption("Cálculo matemático exacto basado en tu plantilla.")
        calc_cuota = st.number_input("Simular Cuota:", min_value=1.01, value=1.39, step=0.01)
        
        # --- FÓRMULAS DE TU EXCEL ---
        # 1. Porcentaje de probabilidad implícita
        prob_implicita = (1 / calc_cuota) * 100
        
        # 2. Inversión: (Bankroll * Porcentaje) / 6.5
        sim_inv = (balance_total * (prob_implicita / 100)) / 6.5
        
        # 3. Stake (Cuota / 1)
        sim_stake = round(calc_cuota / 1)
        
        st.info(f"Prob. **{prob_implicita:.0f}%** ➔ **Stake {sim_stake}** ➔ Invertir aprox: **$ {sim_inv:.2f}**")
